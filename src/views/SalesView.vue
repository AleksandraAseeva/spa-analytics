    ...
    <div class="layout">
      ...
      <div class="right-panel">
        <div class="chart-container">
          ...
          <ChartComponent :data="chartData" chartType="line" />
        </div>

        <div class="table-container">
          <TableComponent :data="tableData" pageType="sales" />
        </div>
      </div>
    </div>
  </div>
...
const filteredProducts = computed(() => {
  return products.value.filter((product) => {
    ...
    const matchesSearch =
      product.price_with_disc.toLowerCase().includes(searchLower) ||
      ...
      (product.region_name &&
        product.region_name.toString().includes(searchQuery.value));
    ...
    const matchesWarehouse = selectedWarehouse.value
      ...
      : true;
    return matchesSearch && matchesBrand && matchesWarehouse && matchesCategory;
  });
});
...
const tableData = computed(() => {
  return [...filteredProducts.value]
    .sort((a, b) => parseFloat(b.finished_price) - parseFloat(a.finished_price))
    .map((item) => ({
      ...item,
      name: item.warehouse_name,
      finished_price: parseFloat(item.finished_price),
    }));
});
